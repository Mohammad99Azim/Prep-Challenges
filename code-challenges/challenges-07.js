...
const objLat = (obj) => {
    ...
    obj.firstName = obj.firstName[0].toUpperCase() + obj.firstName.substring(1);
    obj.lastName = obj.lastName[0].toUpperCase() + obj.lastName.substring(1);
...
const classesAvg = (data) => {
    ...
    for (let i = 0; i < data.grades.length; i++) {
        ...
        for (let x = 0; x < data.grades[i].numberOfClasses; x++) {
            ...
            let sum = 0;
            let studentNum = data.grades[i].classes[x].classScores.length;

            for (let y = 0; y < data.grades[i].classes[x].classScores.length; y++) {
                sum += data.grades[i].classes[x].classScores[y];
            }
            data.grades[i].classes[x].avg = Math.floor(sum / studentNum);
