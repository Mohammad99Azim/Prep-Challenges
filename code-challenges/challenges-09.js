...
const roundDecimals = (arr) => {
        arr.forEach(function(part, index, theArray) {
            theArray[index] = Math.round(theArray[index]);
        });
        return arr;
    }
    // -------------------------------------------------------------------------------------------------------
...
const employeesBonus = (arr) => {
        ...
        arr.forEach(function(part, index, theArray) {
            if (theArray[index].workHours > 8) {
                let val = parseInt(theArray[index].salary);
                val += 100;
                theArray[index].salary = val + "$";
            } else {
                let val = parseInt(theArray[index].salary);
                val += 50;
                theArray[index].salary = val + "$";
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
        let most = 0;
        mouseArray.forEach(function(part, index, theArray) {
            keyBoardArray.forEach(function(part2, index2, theArray2) {
                if ((theArray[index] + theArray2[index2]) > most && (theArray[index] + theArray2[index2]) <= budget) {
                    most = (theArray[index] + theArray2[index2]);
                }
            });
        });
        return most;
