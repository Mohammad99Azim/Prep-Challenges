...
const customerAndAge = (obj) => {
    let custmArr = [];
    let kyes = Object.keys(obj);
    let valuess = Object.values(obj);
    for (let i = 0; i < kyes.length; i++)
        custmArr.push(`Customer Name :${kyes[i]} , Age :${valuess[i]}`);
    return custmArr;
};
// -------------------------------------------------------------------------------------------------------
...
const getEntries = (obj) => {
    ...
    let valuess = Object.values(obj);
    for (let i = 0; i < kyes.length; i++)
        custmArr.push(`${kyes[i]}: ${valuess[i]}`);
    return custmArr;
};
...
const getInfo = (arr) => {
    ...
    for (let i = 0; arr.length > i; i++) {
        ...
        for (let x = 0; arr[i].Students.length > x; x++) {
            studentsName.push(arr[i].Students[x]);
        }
    }
...
const getStudents = (arr) => {
    ...
    for (let y = 0; arr.length > y; y++) {
        for (let i = 0; courses.length > i; i++) {
            if (courses[i].Students.includes(arr[y])) {
                studnets.push({
                    Student: `${arr[y]}`,
